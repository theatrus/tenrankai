...
async fn test_asset_url_filter_in_templates() {
    ...
    fs::write(static_dir.join("test.css"), "/* test css */").await.unwrap();
    fs::write(static_dir.join("test.js"), "// test js").await.unwrap();
...
<body>
    ...
    fs::write(template_dir.join("test.html.liquid"), template_content).await.unwrap();
    ...
    let result = template_engine.render_template("test.html.liquid", globals).await.unwrap();
    
    // Verify the output contains versioned URLs
    assert!(result.contains("test.css?v="), "CSS should have version parameter");
    assert!(result.contains("test.js?v="), "JS should have version parameter");
    assert!(result.contains("/static/test.css?v="), "CSS should have full path with version");
    assert!(result.contains("/static/test.js?v="), "JS should have full path with version");
}
...
async fn test_asset_url_filter_with_page_css_and_js() {
    ...
    // Create test CSS and JS files
    fs::write(static_dir.join("page.css"), "/* page css */").await.unwrap();
    fs::write(static_dir.join("app.js"), "// app js").await.unwrap();
...
{% endfor %}"#;
    ...
    fs::write(template_dir.join("page.html.liquid"), template_content).await.unwrap();
    ...
    let result = template_engine.render_template("page.html.liquid", globals).await.unwrap();
    
    // Verify the output
    assert!(result.contains("/static/page.css?v="), "page.css should have versioned URL");
    assert!(result.contains("/static/app.js?v="), "app.js should have versioned URL");
}