...
fn test_avif_gain_map_preservation() {
    ...
    assert!(avif_info.has_gain_map, "Original image should have gain map");
    assert!(avif_info.gain_map_info.is_some(), "Gain map info should be present");
    
    let original_gain_map = avif_info.gain_map_info.as_ref().unwrap();
    assert!(original_gain_map.has_image, "Gain map should have image data");
    assert!(original_gain_map.gain_map_image.is_some(), "Gain map image should be present");
    ...
    assert_eq!(img.width(), original_gm_width, "Gain map width should match main image");
    assert_eq!(img.height(), original_gm_height, "Gain map height should match main image");
    ...
    assert!(output_info.has_gain_map, "Output image should have gain map");
    assert!(output_info.gain_map_info.is_some(), "Output gain map info should be present");
    
    let output_gain_map = output_info.gain_map_info.as_ref().unwrap();
    assert!(output_gain_map.has_image, "Output gain map should have image data");
    assert!(output_gain_map.gain_map_image.is_some(), "Output gain map image should be present");
    ...
    assert_eq!(output_info.transfer_characteristics, avif_info.transfer_characteristics);
    assert_eq!(output_info.is_hdr, avif_info.is_hdr);
    
    // Verify gain map parameters were preserved (with floating point tolerance)
    let epsilon = 0.001;
    for i in 0..3 {
        assert!((output_gain_map.gamma[i] - original_gain_map.gamma[i]).abs() < epsilon,
                "Gamma[{}] mismatch: {} vs {}", i, output_gain_map.gamma[i], original_gain_map.gamma[i]);
        assert!((output_gain_map.min[i] - original_gain_map.min[i]).abs() < epsilon,
                "Min[{}] mismatch: {} vs {}", i, output_gain_map.min[i], original_gain_map.min[i]);
        assert!((output_gain_map.max[i] - original_gain_map.max[i]).abs() < epsilon,
                "Max[{}] mismatch: {} vs {}", i, output_gain_map.max[i], original_gain_map.max[i]);
        assert!((output_gain_map.base_offset[i] - original_gain_map.base_offset[i]).abs() < epsilon,
                "Base offset[{}] mismatch: {} vs {}", i, output_gain_map.base_offset[i], original_gain_map.base_offset[i]);
        assert!((output_gain_map.alternate_offset[i] - original_gain_map.alternate_offset[i]).abs() < epsilon,
                "Alternate offset[{}] mismatch: {} vs {}", i, output_gain_map.alternate_offset[i], original_gain_map.alternate_offset[i]);
    }
    assert!((output_gain_map.base_hdr_headroom - original_gain_map.base_hdr_headroom).abs() < epsilon,
            "Base HDR headroom mismatch: {} vs {}", output_gain_map.base_hdr_headroom, original_gain_map.base_hdr_headroom);
    assert!((output_gain_map.alternate_hdr_headroom - original_gain_map.alternate_hdr_headroom).abs() < epsilon,
            "Alternate HDR headroom mismatch: {} vs {}", output_gain_map.alternate_hdr_headroom, original_gain_map.alternate_hdr_headroom);
}
...
fn test_avif_without_gain_map() {
    ...
    // Verify no gain map
    assert!(!read_info.has_gain_map, "Regular AVIF should not have gain map");
    assert!(read_info.gain_map_info.is_none(), "Gain map info should be None");
