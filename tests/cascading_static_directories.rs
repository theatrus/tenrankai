...
async fn test_cascading_static_directories() {
    ...
    fs::write(static_dir1.join("shared.css"), "/* from static1 */").await.unwrap();
    fs::write(static_dir1.join("file1.js"), "// file1 from static1").await.unwrap();
    
    fs::write(static_dir2.join("shared.css"), "/* from static2 */").await.unwrap(); 
    fs::write(static_dir2.join("file2.js"), "// file2 from static2").await.unwrap();
    
    // Create a minimal template
    fs::write(templates_dir.join("index.html.liquid"), "<h1>Test</h1>").await.unwrap();
    ...
    assert_eq!(shared_content, "/* from static1 */", "Should serve from first directory");
...
async fn test_favicon_cascading_directories() {
    ...
    fs::write(static_dir2.join("favicon.svg"), favicon_content).await.unwrap();
    ...
    assert_eq!(favicon_response.header("content-type"), "image/vnd.microsoft.icon");
