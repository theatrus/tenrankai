...
pub fn read_avif_info(path: &Path) -> Result<(DynamicImage, AvifImageInfo), ImageError> {
    // Read the file
...
pub fn read_avif_info_from_bytes(data: &[u8]) -> Result<(DynamicImage, AvifImageInfo), ImageError> {
    // Check if it's a valid AVIF file
    if !is_avif(data) {
        return Err(ImageError::Processing("Not a valid AVIF file".to_string()));
    }
