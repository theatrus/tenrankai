...
pub struct GainMapInfo {
    pub gamma: Option<f32>,  // Gamma value for gain map application
    pub min_boost: Option<f32>,  // Minimum boost factor
    pub max_boost: Option<f32>,  // Maximum boost factor
    pub base_offset: Option<f32>,  // Base offset for tone mapping
    pub alternate_offset: Option<f32>,  // Alternate offset for tone mapping
}
...
pub fn read_avif_info(path: &Path) -> Result<(DynamicImage, AvifImageInfo), GalleryError> {
    ...
    unsafe {
        ...
        // Check for gain map presence by parsing the AVIF container
        // Note: This is a container-level check since libavif 1.0.4 doesn't have gain map API
        debug!("Starting gain map detection for AVIF container");
        let (has_gain_map, gain_map_info) = detect_gain_map_in_container(&data);
        debug!("Gain map detection completed: has_gain_map={}", has_gain_map);
...
fn read_with_fallback(path: &Path) -> Result<(DynamicImage, AvifImageInfo), GalleryError> {
    ...
    let info = AvifImageInfo {
        ...
        has_gain_map: false,  // Fallback can't detect gain maps
        gain_map_info: None,  // No gain map info in fallback
    };
...
pub fn detect_gain_map_in_container(data: &[u8]) -> (bool, Option<GainMapInfo>) {
    debug!("detect_gain_map_in_container called with {} bytes", data.len());
    ...
    if has_tmap_anywhere {
        ...
        let gain_map_info = Some(GainMapInfo {
            gamma: None,  // Would need libavif 1.1.1+ to extract
            min_boost: None,  // Would need libavif 1.1.1+ to extract
            max_boost: None,  // Would need libavif 1.1.1+ to extract
            base_offset: None,  // Would need libavif 1.1.1+ to extract
            alternate_offset: None,  // Would need libavif 1.1.1+ to extract
        });
    ...
    while pos + 8 <= data.len() {
        ...
        let box_type = &data[pos+4..pos+8];
        debug!("Found box type: {:?} at position {}, size {}", 
               std::str::from_utf8(box_type).unwrap_or("invalid"), pos, box_size);
        
        // Look for meta box which contains item references
        if box_type == b"meta" && pos + 12 < data.len() {
            debug!("Processing meta box at position {}", pos);
            if let Some(gain_map_detected) = detect_gain_map_in_meta(&data[pos+12..pos+box_size]) {
    ...
    // Note: We can't extract detailed gain map parameters without libavif 1.1.1+ API
...
fn detect_gain_map_in_meta(meta_data: &[u8]) -> Option<bool> {
    ...
    while pos + 8 <= meta_data.len() {
        ...
        let box_type = &meta_data[pos+4..pos+8];
        debug!("Meta box - Found box type: {:?} at position {}, size {}", 
               std::str::from_utf8(box_type).unwrap_or("invalid"), pos, box_size);
        ...
        if (box_type == b"iprp" || box_type == b"ipco" || box_type == b"iref") && box_size > 8 {
            if let Some(result) = detect_gain_map_in_meta(&meta_data[pos+8..pos+box_size]) {
                return Some(result);
            }
        }
...
fn extract_aux_type_from_auxc(auxc_data: &[u8]) -> Option<String> {
    ...
    if version_and_flags[0] != 0 {  // Only version 0 is supported
        return None;
    }
    ...
    if let Some(null_pos) = aux_type_data.iter().position(|&b| b == 0) {
        if let Ok(aux_type) = std::str::from_utf8(&aux_type_data[..null_pos]) {
