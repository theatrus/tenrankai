...
pub use types::{ImageUserMetadata, PickStatus, Comment};
...
pub trait MetadataStorage: Send + Sync {
    ...
    async fn load(&self, image_path: &Path) -> Result<Option<ImageUserMetadata>, MetadataStorageError>;
    
    /// Save metadata for a specific image
    async fn save(&self, image_path: &Path, metadata: &ImageUserMetadata) -> Result<(), MetadataStorageError>;
    ...
    async fn load_batch(&self, image_paths: &[&Path]) -> Result<Vec<(String, ImageUserMetadata)>, MetadataStorageError> {
