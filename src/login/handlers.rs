...
pub async fn login_page(
    ...
    match app_state.template_engine.render_template("login.html.liquid", globals).await {
...
pub async fn login_request(
    ...
    // Build login URL
    let base_url = app_state.config.app.base_url.as_deref().unwrap_or("http://localhost:8080");
...
pub async fn verify_login(
    ...
    let username = {
        let mut login_state = app_state.login_state.write().await;
        login_state.verify_token(&query.token)
...
pub async fn login_success(
    ...
    match app_state.template_engine.render_template("login_success.html.liquid", globals).await {
        Ok(html) => Ok(Html(html)),
        Err(e) => {
