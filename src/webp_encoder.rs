...
impl WebPEncoder {
    ...
    pub fn encode(&self, quality: f32, icc_profile: Option<&[u8]>) -> Result<Vec<u8>, WebPError> {
        unsafe {
            ...
            let mut config = libwebp_sys::WebPConfig::new()
                .map_err(|_| WebPError::EncodingFailed)?;
            ...
            // Import RGB data
            if libwebp_sys::WebPPictureImportRGB(&mut picture, self.rgb_data.as_ptr(), (self.width * 3) as i32) == 0 {
    ...
    unsafe fn add_icc_profile_with_mux(&self, webp_data: &[u8], icc_profile: &[u8]) -> Result<Vec<u8>, WebPError> {
        unsafe {
...
mod tests {
    ...
    fn test_encoding_with_icc_profile() {
        ...
        while pos + 8 <= webp_data.len() {
            ...
            let chunk_size = u32::from_le_bytes([
                webp_data[pos + 4], webp_data[pos + 5], 
                webp_data[pos + 6], webp_data[pos + 7]
            ]) as usize;
    ...
    fn create_test_icc_profile() -> Vec<u8> {
        ...
        vec![
            ...
            b'A', b'D', b'B', b'E',  // Platform signature  
            ...
            b'T', b'E', b'S', b'T',  // Profile creator
            // Reserved (44 bytes)
            ...
            b'T', b'e', b's', b't', b' ', b'P', b'r', b'o', b'f', b'i', b'l', b'e', 0x00,
